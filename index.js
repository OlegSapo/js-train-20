...
Car.prototype.toString = function() {
    return `Brand: ${this.brand}, Model: ${this.model}, Year: ${this.year}, FuelType: ${this.fuelType}`;
  };
...
// Метод brake для гальмування прототипу Car,зменшує this.speed на передане число та виводить рядок в консоль в консоль: Автомобіль <brand> <model> зменшив до швидкості <speed> км/год
 Car.prototype.brake = function(decr) {
  this.speed-=decr;
  console.log(`Автомобіль ${this.brand} ${this.model} пригальмував до швидкості ${this.speed} км/год.`);
  };
}
...
function Truck(
  ...
  } else {console.log(`Тягнення навантаження ${cargo}`);}
}
}
...
this.mileage+=dist;
console.log(`Подорожуємо ${this.mileage} km у ${this.brand} ${this.model}`);
};
...
let driveKM = car.drive.bind(car);
driveKM(100);
